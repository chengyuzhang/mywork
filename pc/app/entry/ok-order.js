...
import {rand,signName,reTop,tabBar,findArrIndex,imgLazy,setSto,getSto,cartCount,rmSto,allSto,url_search} from '../js/config';
...
if(url_search().from=='custom'){
    ...
    //优惠券
	(function(){
		var iTotalPrice=getSto('order-money').slice(1);
	    var oPacity=$('.opacity');
	    ...
	    $(oBtn).on('click',function(){
	        ...
	        $.ajax({
	            ...
	            data:JSON.stringify({
	                ...
	                payType:1,
	                coupon:$('.coupon input').val(),
	                comment:$('.remarks ul .words textarea').val(),//订单评论
			...
			(function(){
			    ...
			    oText.on('input',function(){
			        ...
			        if(val.length<9){
			            ...
			            $(oCouponPrice1).text('-￥00');
			            $(oCouponPrice2).text('-￥00');
			            $(oLastPrice).text('￥'+parseInt(iTotalPrice));
			        }else{
			            $(oText).val(text);
			        }
			        if(val.length==8){
