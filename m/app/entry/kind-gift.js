	...
	aBtn.each(function(index,item){
		...
		$(item).on('click',function(){
			...
			if(item.dataset.btn=='true'){
				...
				whoIndex=index;
				item.dataset.btn='false';
				jsonData.targetIds=item.dataset.id;
			}else{
			...
			aBtn.each(function(index,item){
				...
				$(item).on('click',function(){
					...
					if(item.dataset.btn=='true'){
						...
						whatIndex=index;
						item.dataset.btn='false';
						jsonData.labelIds=item.dataset.id;
					}else{
						$(this).removeClass('active');
						$('.for-what').get(0).dataset.can="true";//恢复庆祝什么可以点击
						whatIndex=-1;
			...
			aBtn.each(function(index,item){
				...
				$(item).on('click',function(){
					...
					if(item.dataset.btn=='true'){
						...
						stylesIndex=index;
						item.dataset.btn='false';
						jsonData.styleIds=item.dataset.id;
					}else{
						$(this).removeClass('active');
						stylesIndex=-1;
						item.dataset.btn='true';
			...
			aBtn.each(function(index,item){
				...
				$(item).on('click',function(){
					...
					if(item.dataset.btn=='true'){
						...
						item.dataset.btn='false';
						jsonData.materialIds=item.dataset.id;
					}else{
						$(this).removeClass('active');
						materialsIndex=-1;
						item.dataset.btn='true';
			...
			aBtn.each(function(index,item){
				...
				$(item).on('click',function(){
					...
					if(item.dataset.btn=='true'){
						...
						pricesIndex=index;
						item.dataset.btn='false';
						jsonData.priceIds=item.dataset.id;
					}else{
						$(this).removeClass('active');
						pricesIndex=-1;
						item.dataset.btn='true';
            	...
            	if(arg=='child'){
            		if(index==0){
			            ...
			            str+=`<li class="goods-children-item-public">
			                <div>`;
			                    if(item.goodsList[0]){
				                    str+=`<a href="product-details.html?id=${item.goodsList[0].goodsId}"><img src="" data-src=${item.firstPicture} alt=""></a>`;
				                }
				                if(item.goodsList[1]){
					              	str+=`<a href="product-details.html?id=${item.goodsList[1].goodsId}"><img src="" data-src=${item.secondPicture} alt=""></a>`;
					          	}
			                    str+=`<p>${item.title}</p> 
			                </div>
			            </li>`;
