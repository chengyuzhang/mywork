...
import '../css/my-order-wait-get.css';
import $ from 'n-zepto';
...
//删除订单详情页面的记录上一个地址
//console.log(getSto('prevurl'));
if(getSto('prevurl')){console.log(12345);
	rmSto('prevurl');
}
		                    ...
		                    if(statusCodeControl(item.statusCode).length){
			                    for(var i=statusCodeControl(item.statusCode).length-1; i>=0; i--){
			                    	if(statusCodeControl(item.statusCode)[i]=='确认收货'){
			                    		...
			                    		str+='<em class="check" data-order='+item.orderNo+'><a href="custom-detail.html?orderNo='+item.orderNo+'&statusCode='+item.statusCode+'#express">'+statusCodeControl(item.statusCode)[i]+'</a></em>';
			                    	}else if('联系客服'){
