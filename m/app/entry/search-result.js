...
(function(){
	...
	$.ajax({
		url:apiUrl+'/search?keyword='+valObject.keyword,
		...
		success:function(data){
			...
			if(!data.body.content.content.length){
				$('.result').html('<img style="width:100%;" src="'+require('../imgs/no-search.png')+'" alt=""><p>抱歉，没有找到内容呃~</p>');
			}

			init(data.body.content.content);
		},
		error:function(err){
			console.log(err);
		}
	...
	//load();
	//页面初始化
	function init(arrShow){
		var oUl=$('.result ul');
		//首页商品展示前3条
		arrShow.forEach(function(item,index){
			str+='<li class="item search-result"><section class="block">';
			str+='<a href="detail.html?id='+item.id+'"><div class="item-head"><img class="avater" src="'+item.avatar+'" ><span class="name">'+item.nickName+'</span><i class="time">'+item.created+'</i></div>';
			str+='<div class="img-wrap"><img class="show-pic" src="'+item.picture+'" ></div>';
			str+='<h2 class="title">'+item.articleName+'</h2>';

			str+='<p class="text search-p">'+item.articleDesc+'</p></a>';
		...
		// 	str+='<p class="text">'+item.content+'</p></a>';
		// 	str+='<div class="item-foot"><span class="words">留言 '+item.messageNum+'</span><i></i><span class="good">点赞 '+item.interestedNum+'</span>';
	...
	function refresh(arrShow){
		...
		oUl.html(str);
		var aImg=$('.show-pic');console.log(aImg);
		imgPos(aImg);
	}
