...
import $ from 'n-zepto';
import {rand,signName,prevUrl,url_search,rmSto} from '../js/config';
import md5 from 'md5';
//接口地址
...
(function(){
	...
	$('.list li').eq(0).find('input').val(valObject.name);
	$('.list li').eq(1).find('input').val(valObject.tel);
	$('.list li').eq(2).find('input').val(valObject.zone);
	$('.list li').eq(3).find('input').val(valObject.address);
	$('.list').get(0).dataset.id=valObject.id;
})();
...
(function(){
	...
	oBtn.on('click',function(){
		...
		if((name&&tel&&zone&&address)!=''){
			...
			console.log('xyz:',valObject.defaultAddress);
			$.ajax({
				type:'post',
				...
				data:{
					consignee:name,
					mobile:tel,
					defaultAddr:valObject.defaultAddress,
					zone:zone.replace(/\//, ""),
					detail:address,
					memberNo:vipNo,
		   ...
		   //          }
					...
					if(url_search().from=='ok-order'){
						window.location.href='manage-address.html?type='+url_search().type+'&to=manage-address&jump='+valObject.jump+'&vipId='+valObject.vipId+'&f='+url_search().f;
					}else{
						window.location.href='manage-address.html?jump='+valObject.jump+'&vipId='+valObject.vipId;
					}
				},
				error:function(err){
					alert('添加失败！');
...
// 		valObject=url2json(temp.split('?')[1]);
// 	}
// 	$('header a').attr('href','manage-address.html?jump='+valObject.jump+'&vipId='+valObject.vipId);
// })();
//返回上一页
(function(){
	// var temp=window.location.search;
