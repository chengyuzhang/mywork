...
(function(){
	...
	var valProv='北京市';
	var valCity='';
	var oInput=$('.local-address input');
	//默认显示北京区的
...
//控制手机号码位数小于11位
(function(){
	var oBtn=$('.order-tel>input');
	var oBtn1=$('.geter-tel>input');
	var text1='';
	var text2='';

	oBtn.on('input',function(){
		if($(oBtn).val().length<12){
			text1=$(oBtn).val();
		}else{
			$(oBtn).val(text1);
		}
	...
	oBtn1.on('input',function(){
		if($(oBtn1).val().length<12){
			text2=$(oBtn1).val();
